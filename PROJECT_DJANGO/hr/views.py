        ...
        if request.POST.get('user_input'):
            ...
            if user_input:
                ...
                try:
                    ...
                    # )
                    # print(index_hr)
                    query_engine = index_hr.as_query_engine(similarity_top_k=3, node_postprocessors=[rerank],
                    ...
                    # print(timezone.now().strftime('%Y-%m-%d'))
                    # response__2 = index_hr.as_query_engine(text_qa_template=text_qa_template).query(query_2)
    ...
    for line in lines:
        ...
        if bold_match:
            ...
            formatted_lines.append(
                f'<p style="font-weight: bold; text-align: justify;">{current_number}. {bold_match.group(2).strip()}</p>')
            ...
            if not in_list or list_type != 'ul':
                ...
                list_type = 'ul'
            formatted_lines.append(
                f'<li style="margin-left: 50px; text-align: justify;">{numbered_match.group(2).strip()}</li>')
            ...
            if asterisk_items:
                for item, _ in asterisk_items:
                    formatted_lines.append(f'<li style="margin-left: 50px; text-align: justify;">{item.strip()}</li>')
            else:
                formatted_lines.append(f'<li style="text-align: justify;">{asterisk_match.group(2).strip()}</li>')
            ...
            formatted_lines.append(f'<p style="text-align: justify;">{line.strip()}</p>')
